...
| O43 | **The forest pre-emption line differs between two documents.** `19` §2a.2 writes it in the genitive with no brackets; the test plan hashes a bracketed slot | **Open, needs you.** A citation record cannot inflect a Polish noun, so the genitive form defeats the point of rendering the holder from the record. The code follows the hashed template. Ratify one wording |
| O44 | **Which regimes ship the degraded badge?** The test plan §6.4 says both; `19` §2a and D122 say the forest badge is blocked | **Open, needs you.** The code follows `19`: forest refuses to render, farmland renders degraded. The distinction is that the farmland act and holder sit in ratified copy while the forest ones are `‡` claims |
| O45 | **The re-verification prompt copy is ungrammatical.** The verb takes the accusative, so a slot holding an act title in the nominative reads wrongly | **Open.** The template is proposed, not ratified. Worth fixing before it lands in `21` |
| O46 | **No document names the regulation behind the register-class list.** The 24 classes come from the test plan, and nothing says which regulation defines them | **Open, needs you.** `config/register_classes.yml` records the absence as `⟨RECORD⟩` rather than a remembered citation. The badge's regime map rests on this list |
| O47 | **The coverage record's staleness limit has no ratified value and no key.** Nothing says how old a coverage check may be before it stops counting as evidence | **Open, raised by S14.** The code makes it a required argument with no default, so it cannot be forgotten quietly. It needs a value and a row in `params.yml` |
| O48 | **The shipped coverage-probe values and the test scene's working values differ.** `params.yml` ships 500 m and 3 buildings; the scene needs 2 000 m and 5 | **Open, and expected.** D103 made both configurable precisely because the 20-parcel labelled set has not arbitrated them. Under the shipped values one scene parcel reads `unknown` rather than `unlikely`. A test records the gap rather than hiding it |
| O49 | **Which OSM highway classes count as a public road?** D114 accepts "an OSM highway class" as half the proxy without naming which | **Open, raised by S14.** The set in code is marked unratified. It decides which roads produce a `likely` verdict, so it is not a detail |
| O50 | **Cell 1 of the truth table has no reason code.** The documents write "—", and the schema column is `NOT NULL` | **Open.** S14 named it `good_neighbour_satisfied`. That name is the agent's, not ratified |
| O51 | **The affirmative-styling ban forbids a phrase the ratified disclaimer contains.** The scan forbids *gwarancja wydania* on a `likely` page, and the ratified disclaimer says *nie jest to gwarancja wydania WZ* | **Open.** The scan exempts the disclaimer line and says so. One of the two texts should change |
